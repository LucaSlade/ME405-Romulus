...
One of the most impactful discoveries was the significant impact of battery charge on performance and reliability. When the battery voltage was high, the robot moved faster and responded more predictably. However, as the charge decreased, the motors became sluggish, sensor readings varied more, and timing inconsistencies emerged—especially in tasks relying on precise movements.
...
Overall, our biggest difficulty came from working on too many tasks at once. We did not realize that the coding structure we had implemented in the previous labs for the individual tasks was done in a manner that was slow and inefficient. When combining the functionality for the final lab we found the scheduler to be increasingly difficult. The average late was high for many tasks and it seemed that there were tasks that were robbing time. In my mind, the thinker should be the highest priority but have the biggest period as it takes longer to run. Conversely, the bumper sensor and user interface could be much lower priority and smaller frequency. I believe that our code has issues with improper flag setting and communication. When I attempted to debug, I noticed that the print statement and various other calls for information slowed down the code and affected the scheduler. This I especially noticed with the encoder. Additionally, due to times of not saving and working when the Romi battery was low inconsistencies were brought into the code. For the future, I would emphasize the importance of the design in the beginning. I would have checked with other classmates or Professor Refvem earlier on. Additionally, in our initial design we had attempted to create a code structure which would be more modular in that we did not have the sequential state transition like we do now. This was a severe waste of time as flags were being set in various locations and never updates properly. Overall, our problems came from integrating the various tasks. We were able to get the romi to complete all the necessary actions yet failed in producing one whole code. 
